...
/**
 * Añade `message.participant_added` y `message.participant_removed`.
 *
 * Un hilo se ve si estás dentro de él, también si eres administrador — ver
 * App\Support\Messaging\MessageScope. Eso deja una puerta legítima y una sola:
 * **añadirse al hilo**. Y esa puerta tiene que dejar rastro, porque si no la
 * regla no vale nada: bastaría meterse, leer y salirse.
 *
 * De las 59 acciones que hay no sirve ninguna. `permission.changed` habla de la
 * matriz de permisos, no de quién entra en una conversación, y usarla mentiría
 * en la pantalla de bitácora.
 *
 * Como las de `driver.verified` y `lead.*`, con migración y no reescribiendo
...
{
    /** Las 59 anteriores, más las dos nuevas. */
    ...
    private const NUEVAS = ['message.participant_added', 'message.participant_removed'];

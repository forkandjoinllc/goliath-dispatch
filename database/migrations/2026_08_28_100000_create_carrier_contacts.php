...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // igual que en el resto del esquema: la columna generada vale NULL salvo
        // para el principal vivo, y MySQL ignora los NULL en un índice único.
        DB::statement('
            alter table carrier_contacts
            add column `live_primary_key` char(36)
            generated always as (
                case when `deleted_at` is null and `is_primary` = 1 then `carrier_id` end
            ) stored
        ');

        DB::statement('
            alter table carrier_contacts
            add unique key `carrier_contacts_primary_uq` (`tenant_id`, `live_primary_key`)
        ');

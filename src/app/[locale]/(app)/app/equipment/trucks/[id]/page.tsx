...
export default async function TruckDetailPage({
  ...
  const resource = await getEquipmentResourceContext(tenantDb(actorPreview.tenantId), 'truck', id)
  const ctx = await loadFor('equipment:read', resource)
  const detail = await getEquipmentDetail(ctx.db, 'truck', id)
  ...
  const [mediaItems, history] = await Promise.all([
    ...
    listEquipmentAuditHistory(ctx.db, 'truck', id),
  ])

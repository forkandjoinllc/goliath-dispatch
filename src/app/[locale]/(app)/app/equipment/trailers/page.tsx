...
import { listTrailers } from '@/server/equipment/queries'
...
export default async function TrailersPage({
  ...
  const result = await listTrailers(ctx.db, scope, {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t('equipment.trailers.title')}
        primaryAction={
          canCreate ? (
            <Button asChild>
              <Link href={`/${locale}/app/equipment/trailers/new`}>{t('equipment.trailers.new')}</Link>
            </Button>
          ) : undefined
        }
      />

...
export default async function NewTrailerPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [{ carriers }, types] = await Promise.all([
    ...
    listEquipmentTypes(ctx.db, 'trailer'),
  ])

  return (
    <div className="space-y-6">
      <PageHeader title={t('equipment.trailers.new')} />
      <EquipmentForm
        locale={locale}
